...
describe('stripUnchanged() 테스트', () => {
  describe('args로 받은 프로퍼티중 값이 변경되지 않은 프로퍼티들만 반환한다.', () => {
    const human = new Human({
      ...
      name: 'arthur',
      career: [{ name: 'parrot world', stack: ['javascript', 'typescript'], yearsOfEx: 1 }],
      like: ['game', 'coding', 'eat'],
      pet: { profile: { name: 'junbae', breed: 'parrot' }, age: 8 },
    });
    ...
    test('전부', () => {
      const stripped = human.stripUnchangedProperty({
        ...
        name: 'hwang',
        career: [{ name: 'parrot world', stack: ['javascript', 'typescript', 'react'], yearsOfEx: 1 }],
        like: ['game', 'eat'],
