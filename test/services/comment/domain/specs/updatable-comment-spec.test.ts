...
import { UpdatableCommentSpec } from '@comments/domain/specs';
...
describe('UpdatableCommentSpec 테스트', () => {
  ...
  describe('find test', () => {
    describe('권한 테스트', () => {
      test('관리자의 경우 comment을 반환한다.', async () => {
        ...
        const spec = new UpdatableCommentSpec({ user: adminUser, id: 'commentId' });
      ...
      test('관리자가 아닌 경우 작성자만 comment을 반환한다.', async () => {
        ...
        const spec = new UpdatableCommentSpec({ user: author, id: 'commentId' });
      ...
      test('작성자가 아닌 경우 에러를 던져야 한다.', async () => {
        ...
        const spec = new UpdatableCommentSpec({ user, id: 'commentId' });
        ...
        try {
          ...
          expect(err).toEqual(
            forbidden(`User is not allowed to update the comment(commentId).`, {
